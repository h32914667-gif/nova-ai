...
export default function Subscription({ userId, onClose }) {
  ...
  async function handleUpgrade(planKey) {
    ...
    if (!confirm(`Перейти на тариф ${plans[planKey].name} за ${plans[planKey].price} ₽/мес?`)) return;
    try {
      ...
      <div className="w-full max-w-4xl bg-white/10 backdrop-blur-2xl rounded-3xl border border-white/20 shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
        ...
        {currentPlan && (
          <div className="mb-6 p-4 bg-white/10 rounded-2xl border border-white/10">
            <p className="text-lg text-white">Текущий тариф: <span className="font-bold text-indigo-400">{plans[currentPlan.plan]?.name || currentPlan.plan}</span></p>
            {currentPlan.expires_at && (
              <p className="text-sm text-slate-400">Действует до: {new Date(currentPlan.expires_at).toLocaleDateString()}</p>
            )}
            <p className="text-sm text-slate-400">Сообщений в день: {currentPlan.messagesPerDay === Infinity ? '∞' : currentPlan.messagesPerDay}</p>
          </div>
        )}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Object.entries(plans).map(([key, plan]) => (
            <div key={key} className={`bg-white/5 rounded-2xl p-6 border ${currentPlan?.plan === key ? 'border-indigo-500 shadow-lg shadow-indigo-500/20' : 'border-white/10'} hover:bg-white/10 transition-all duration-300 flex flex-col`}>
              <h3 className="text-2xl font-bold text-white">{plan.name}</h3>
              <p className="text-3xl font-bold text-indigo-400 mt-2">{plan.price} ₽<span className="text-sm font-normal text-slate-400">/мес</span></p>
        ...
        <button onClick={onClose} className="mt-6 w-full p-3 rounded-xl bg-white/10 hover:bg-white/20 transition hover:scale-[1.02] text-white">Закрыть</button>
      </div>
    </div>
  );
}